...
public class Reserva {
    ...
    public Reserva(Habitacion habitacion, LocalDate fechaEntrada, LocalDate fechaSalida) {
    ...
    private double calcularPrecio() {
        ...
        double precioBase = noches * habitacion.getPrecioPorNoche();

        // Oferta 2x1: por cada 2 noches, pagas solo 1
        if (noches >= 2) {
            long nochesGratis = noches / 2;
            precioBase -= nochesGratis * habitacion.getPrecioPorNoche();
        }
    ...
    public String toString() {
        return String.format("Reserva - Habitación %d - %s a %s (%d noches) - Total: %.2f€",
                habitacion.getNumero(), fechaEntrada, fechaSalida, getNumeroNoches(), precioTotal);
    }
}
